...
function markdown(options = defaultOptions) {
  ...
  return function markdown(files, metalsmith, done) {
    ...
    matches.forEach(function (file) {
      ...
      if (options.wildcard) {
        keys = expandWildcardKeypath(data, options.keys, '*');
      }
      keys.forEach(key => {
        const value = get__default["default"](data, key);
        if (typeof value === 'string') {
          debug.info('Rendering key "%s" of file "%s"', key.join ? key.join('.') : key, file);
          dset.dset(data, key, options.render(globalRefsMarkdown + value, options.engineOptions, {
            path: file,
            key
          }));
